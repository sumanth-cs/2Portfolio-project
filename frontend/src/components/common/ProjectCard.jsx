    ...
    >
      <div className="relative h-48 overflow-hidden group">
        <img
          ...
          onError={(e) => (e.target.src = placeholder)}
        />
        <div className={`absolute inset-0 bg-black ${isHovered ? 'bg-opacity-50' : 'bg-opacity-0'} flex items-center justify-center gap-4 transition-all duration-300`}>
          {project.codeUrl && (
            <motion.a
              href={project.codeUrl}
              target="_blank"
              ...
              whileHover={{ scale: 1.2 }}
              whileTap={{ scale: 0.9 }}
              className={`p-3 rounded-full bg-white text-black ${isHovered ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}
              style={{ backgroundColor: colors.text, color: colors.background }}
            >
              <Code className="w-5 h-5" />
            </motion.a>
