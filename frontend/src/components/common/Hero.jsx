...
function Hero() {
  ...
  return (
    <motion.section
      id="home"
      className="relative h-[80vh] flex items-center justify-center bg-primary bg-cover bg-center parallax"
      style={{ backgroundImage: 'url(/src/assets/background.jpg)' }}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.8 }}
    >
      <motion.div style={{ opacity }} className="absolute inset-0 bg-black opacity-50"></motion.div>
      <motion.div
        className="relative text-center text-on-primary z-10"
        initial={{ y: 50, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ delay: 0.2, duration: 0.8 }}
      >
        <img
          src="/src/assets/profile.jpg"
          alt="Profile"
          className="w-32 h-32 sm:w-40 sm:h-40 rounded-full mx-auto mb-6 border-4 border-surface"
        />
        <h1 className="text-6xl font-bold mb-4">John Doe</h1>
        <p className="text-2xl mb-6 max-w-md mx-auto">
          Full-Stack Developer | Passionate about building user-friendly and scalable web applications.
