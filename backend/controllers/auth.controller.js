...
export const signup = async (req, res, next) => {
  ...
  try {
    ...
    res.status(201).json({ success: true, token, user: { id: user._id, name: user.name, email, isAdmin: user.isAdmin } });
  } catch (error) {
...
export const login = async (req, res, next) => {
  ...
  try {
    ...
    const token = user.generateToken();
    res.status(200).json({ success: true, token, user: { id: user._id, name: user.name, email, isAdmin: user.isAdmin } });
  } catch (error) {
...
export const getCurrentUser = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ success: true, user: { id: user._id, name: user.name, email: user.email, isAdmin: user.isAdmin } });
  } catch (error) {
